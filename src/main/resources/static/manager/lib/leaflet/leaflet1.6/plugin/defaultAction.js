...
function initDraw(mapName) {
    ...
    window._draw_addToDrawer = function(layer) {
        let getCoordinates = function(ly) {
            let geojson = ly.toGeoJSON();
        ...
        layer.on({
            click: function (e) {
                window.$emit({geoJson:layer.toGeoJSON(),layerId: e.target._leaflet_id},'drawer','click')
            }
        });
    ...
    map.on(L.Draw.Event.CREATED, function (event) {
        window._draw_addToDrawer(event.layer);
    });
}
