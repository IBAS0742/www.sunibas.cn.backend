...
class ParseColumn {
    ...
    add(name,label,parseMethod,setting) {
        ...
        this.setting.push(setting || ParseColumn.setting(180));
    ...
    init() {
        this.btnHeader = `<th lay-data="{field: '___op___',minWidth: ${this.buttonSetting.minWidth},width: ${this.buttonSetting.width}}">操作</th>`;
        this.btnTr = `<td>${
            (_ => {
    ...
    getHead() {
        return `<thead>
                    <tr>
                        ${this.label.map((_,ind) => `<th lay-data="{field:'${this.name[ind]}_${ind}', minWidth: ${this.setting[ind].minWidth}, width: ${this.setting[ind].width} }">${_}</th>`).join('\r\n')}
                        ${this.btnHeader}
                    </tr>
                </thead>`
    }
    ...
    getTable(objs) {
        return `<div id="${this.tableId}"><table lay-filter="${this.tableFilter}">
            ${this.getHead()}
            <tbody>
                ${objs.map(this.getTr.bind(this)).join('\r\n')}
            </tbody>
        </table></div>`;
    }
    getPagination() {
